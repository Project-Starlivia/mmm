...
// 以前はノードの子要素として作っていたため、選ぶ / 外すたびにそのノードの
// 中身が丸ごと作り直され、**コードカードを選ぶだけでコードが再トークナイズ
// されていた**。落とし先の線（drop-line）と同じく、world 座標に浮かぶ
// 1 個の印にすれば、描画はカードの選択を知らなくてよくなる。

import type { Rect } from "./geometry.ts";
import type { CardRef } from "./cards.ts";
...
export class CardPick {
  ...
  constructor() {
    this.kill.append(this.circle, ...this.strokes);
    this.el.append(this.frame, this.kill);
  }

  /**
   * `rect` にぴったり被せる。`ref` は × を押されたときに誰を消すかで、
   * `data-kill` として出す（当たり判定は座標から辿るため）。
   * `rect` が null なら隠す（畳まれた・範囲外）。
   */
  show(ref: CardRef, rect: Rect | null): void {
    if (!rect) {
      this.hide();
      return;
    }
    ...
    const cy = rect.y;
    this.kill.setAttribute("data-kill", `${ref.node},${ref.index}`);
  ...
  hide(): void {
    ...
    this.kill.removeAttribute("data-kill");
  }
}
