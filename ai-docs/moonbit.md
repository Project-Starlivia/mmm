...
19 warning は全部 `struct_never_constructed` / `unused_constructor`（構築子を一度でも使えば消える。実際 `lib/useall.mbt` で全構築子を触ったら 0 warning になった）。相互再帰（Root → Array[Wing] → Wing → Branch → Array[Branch]、Node → Array[Block] → Content）は前方参照も含めて**宣言順に関係なく通る**。
...
```moonbit
let doc = sample()          // roots[0].wings に 1 要素
let r = doc.roots[0]
r.wings.push({ side: Left, branch: leaf(30) })
```
```
P1a doc.roots[0].wings.length = 2
P1a r.wings.length = 2
P1b physical_equal(a, b) = true                      // let a = doc.roots[0]; let b = doc.roots[0]
P1b physical_equal(a.wings, b.wings) = true
...
```moonbit
fn at(doc : Doc, path : Array[Int]) -> Branch {
  let mut branch = doc.roots[path[0]].wings[path[1]].branch
  for k in 2..<path.length() { branch = branch.children[k] }
  branch
}
let n = at(doc, [0, 0])
...
```
```
P3 via return value = 1        // doc.roots[0].wings[0].branch.children.length()
```

4 段の深さでも同じ（`5x`）:
```
5x removed id = 40             // doc.roots[0].wings[0].branch.children[0].children.remove(0)
5x owner len  = 0
5x 元の doc に届いた = 0
5x wings len = 2            // doc.roots[0].wings.insert(0, { side: Left, branch: taken })
5x wings[0].branch.id = 40
```
...
## 2. 入れ子の代入 — **`doc.roots[i].wings[j] = { ..b, side: Left }` は書ける**

```moonbit
let b = doc.roots[0].wings[0]
doc.roots[0].wings[0] = { ..b, side: Left }
```
```
P2a side = Left
P2a branch id kept = 20
```

対して**フィールドへの直接代入は不可**（`lib/neg1.mbt`、その後削除）:
```moonbit
doc.roots[0].wings[0].side = Left
```
```
Error: [4087]
 3 │   doc.roots[0].wings[0].side = Left
   │   ──────────────────┬─────────────────
...
```
5c deep  = n40/true      // amend_wing(doc, [0,0,0,0], set_folded)
5c 中間は無傷 = n30/false
5c wing  = n20/true      // amend_wing(doc, [0,0], ...)
5c root  = (implicit)    // amend_wing(doc, [0], ...) — root は Implicit なので変化なし
```
...
対照型 `MWing { mut side; mut branch }` / `MRoot { mut node; wings }` / `MBranch { mut node; children }` を作って比較。
...
Error: [4036]
 8 │   { ..b, side: Left }
   │   ─────────┬─────────
   │            ╰─────────── Cannot create values of the read-only type: @probe-a/lib.Wing.
Error: [4021]
 8 │   { ..b, side: Left }
   │                ──┬─
   │                  ╰─── The value identifier Left is unbound.
Error: [4036]
14 │   doc.roots[0].wings[0] = { ..b, side: Left }
   │                              ─────────┬─────────
   │                                       ╰─────────── Cannot create values of the read-only type: @probe-a/lib.Wing.
Failed with 6 warnings, 9 errors.
```
```
Error: [4094]（`pub struct MWing` の mut フィールドへの別パッケージからの書き込み）
 3 │   b.side = @lib.Left
   │   ─────────┬────────
...
```
X2 wings after remove = 1     // doc.roots[0].wings.remove(0)
```
...
### (a) 純粋な再構築（`amend_pure` + `amend_branch_pure`）
```moonbit
[i, j, .. rest] => {
  let roots = doc.roots.copy()
  let r = roots[i]
  let wings = r.wings.copy()
  let b = wings[j]
  wings[j] = { ..b, branch: amend_branch_pure(b.branch, rest, f) }
  roots[i] = { ..r, wings, }
  { ..doc, roots, }
}
```
...
match path {
  [] => ()
  [i] => doc.roots[i].node = f(doc.roots[i].node)
  _ => { let n = mbranch_at(doc, path[:]); n.node = f(n.node) }
}
```
```
5b deep  = n40/true
...
- `Root` と `Branch` の両方に `mut node` が要る＝**型が「書き替えられる」と宣言してしまう**
...
O6 中間は無傷 = n3/false
```
書き味: `mut` ゼロ、戻り値なし（`Unit`）、元の doc がその場で更新される。落とし穴は 1 つだけ — **`[i, j]`（翼）の腕だけは Wing と Branch の 2 段を包み直す**:
```moonbit
[i, j] => {
  let b = doc.roots[i].wings[j]
  doc.roots[i].wings[j] = { ..b, branch: { ..b.branch, node: f(b.branch.node) } }
}
```
...
```moonbit
match path { [] => "doc"; [_] => "root"; [_, _] => "wing"; _ => "deep" }
...
**`.. rest` で束縛される `rest` の型は `ArrayView[Int]`**（`Array[Int]` ではない）。`Array[Int]` を取る関数に渡すときは `path[:]` でビューに落とす。`ArrayView` に対しても同じ配列パターンが使える（`amend_branch_pure` / `mbranch_at` で実測）。`rest[0:rest.length() - 1]` のビューの再スライスも通る。
...
## 9. 関数を引数に取る — **`f : (Node) -> Node` の綴りがそのまま通る**

```moonbit
fn amend_sk(sk : Node, f : (Node) -> Node) -> Node { f(sk) }
amend_sk(sk, fold_on)                 // トップレベル関数を名前で渡す
amend_sk(sk, fn(s) { s })             // 匿名関数
let fold = fn(s : Node) -> Node { ... }   // let 束縛（型注釈は要る）
```
```
9 out = H/hello/true/1
9 identity closure ok = true
```
`Array::map` / `filter` へのクロージャ、再帰関数を名前で渡す形（`n.children.map(clone_branch)`）も動く。

---

## 11. 3 つの型を歩く再帰関数（trait 無し）— **素直に書ける**

`Doc → Root → Wing → Branch → Branch` を型ごとに関数を分ける形（`sig_root` / `sig_branch`）で通る。
...
```moonbit
// 構築（呼び出し側は `=`）
Explicit(sign=Heading, label="hello", folded=false, body=[Rule])
// 分解（`~`）
...
O6 root = r1!/true
```
struct 側の punning も通る: `{ side, branch: as_branch(sub) }` / `{ ..doc, roots, }` / `{ ..b, side, }`（**末尾のカンマが要る** — `{ ..doc, roots }` は書けるが `moon fmt` が `{ ..doc, roots, }` に寄せる）。
...
同じ `Sub` を 2 回 graft すると、**中の Branch が物理的に共有される**:
```
A1 shape = doc(R2[>3(4)] R2[>3(4)] R1[] R5[<6])
A1 深部ノードは共有 physical_equal = true
A1 片方を空にすると もう一方 = doc(R2[>3 <77] R2[>3] R1[] R5[<6])   ← 両方から 4 が消えた
```
（`as_root` / `as_branch` が新しい struct と `.map` の新配列を作る層だけは隔離される: `A1 physical_equal(roots[0], roots[1]) = false`、`A1 roots[1].wings len = 1`。**1 段だけ守られて 2 段目から共有**という一番危ない形。）

隔離が要るなら明示の深いコピーが要る（`{ ..n, children: n.children.map(clone_branch) }`、3 関数で書ける）:
```
...
- **腕数は 3 で止まった。** `pluck` 3 腕（roots / wings / children）、`graft` 3 腕 + 変換 2 関数（`as_root` / `as_branch`）、`resolve` 腕なし。move の 9 組合せのうち代表 4 本を実測して全部通った:
  ```
  O2 深い枝 → 別 root の翼   = doc(R1[>2] R5[>3(4) <6])
  O3 翼 → doc 直下（Part→Root 化） = doc(R2[>3(4)] R1[] R5[<6])
  O4 root → 翼（Tree 解体）  = doc(R1[>2(3(4)) <5(6)])
...
- **`amend` だけが 4 腕**（`[]` / `[i]` / `[i, j]` / `[.. head, last]`）。原因は 1 つ — **翼のノードは `Array[Branch]` ではなく `Wing` の中に居る**。`Wing` に `mut branch : Branch` を 1 つ足すだけで 3 腕になる（`opsmut.mbt` の `amend_m` で実測）。**これが「殺す条件」の唯一のきしみ**。今は 4 腕で止まっているので許容範囲だが、`amend` に 5 本目が生えたらこの `mut` 1 個を検討する順序が正しい。
...
2. **pluck / graft と同じ地面に立っている。** pluck/graft は「所有する配列 + index」で書く以外にない（`Array::remove` / `insert`）。amend も同じ座標系で書けるので、`branch_at(doc, head)` という**ヘルパ 1 本を 4 つの道具全部が共有する**。(a) 純粋形はここで座標系が割れる（doc を作り直すので pluck/graft と混ぜられない）。
...
4. **腕は 3 で止まった。** amend の 4 腕目は翼 1 箇所だけで、しかもそれは (b) でも (a) でも構造的に同じ場所（Wing という包み）に生じるコストであり、(c) 固有の負債ではない。
...
**併せて計画に書くべき制約**: (i) `graft` は `at` を clamp する（`insert` の範囲外は catch 不能）、(ii) `Sub` は一度しか graft してはならない（エイリアシング）、(iii) 型は `pub(all)`（別パッケージのテスト / JS 層が構築するなら必須）、(iv) `Root` / `Branch` の `node` を `mut` にする誘惑は `amend` に 5 本目の腕が生えるまで保留。
