...
// 見た目から切り離してここだけを固定する。箱は core に置かせる。
//
// 実行: pnpm test
...
test("箱の中心 → その子", () => {
  assert.deepEqual(drop(centerOf(box(3))), { kind: "node", id: 3, pos: 0 });
});

test("箱の上下の帯 → 直前 / 直後", () => {
  const a = box(3);
  const c = centerOf(a);
  assert.deepEqual(drop({ x: c.x, y: a.y - 10 }), { kind: "node", id: 3, pos: 1 });
  // 下は gap.y ぶんしか空いていない（b がすぐ下）ので、a に近い範囲で当てる
  assert.deepEqual(drop({ x: c.x, y: a.y + a.h + 3 }), { kind: "node", id: 3, pos: 2 });
});
...
test("箱の外側ゾーン（成長する側）→ 子", () => {
  ...
  assert.deepEqual(drop({ x: a.x + a.w + 30, y: c.y }), { kind: "node", id: 3, pos: 0 });
});

test("根の外側 / 根の箱の中（中心より右） → 側・右（子が居る側）", () => {
  const r = box(2);
  const c = centerOf(r);
  assert.deepEqual(drop({ x: r.x + r.w + 10, y: c.y }), { kind: "side", root: 2, left: false });
  assert.deepEqual(drop({ x: r.x + r.w - 5, y: c.y }), { kind: "side", root: 2, left: false });
});

test("根の左外側（子が居ない側） → 側・左", () => {
  const r = box(2);
  const c = centerOf(r);
  assert.deepEqual(drop({ x: r.x - 10, y: c.y }), { kind: "side", root: 2, left: true });
});
...
test("掴んでいる部分木は落とし先から外れる", () => {
  // a(3) を掴んだままその中心へ落としても、a 自身は候補に出ない
  assert.deepEqual(drop(centerOf(box(3)), new Set([3])), { kind: "node", id: 4, pos: 1 });
});
