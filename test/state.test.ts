...
test("highlightRanges — 薄塗りは地図が持つ間だけ。ノードは子孫込み、カードは中身", () => {
  ...
  const held = s.update({ effects: [setHolder.of("map"), setAnchors.of({ kind: "nodes", at: [8], anchor: 8 })] }).state;
  assert.deepEqual(highlightRanges(held), [{ from: 5, to: 10 }]); // a の地番
  const root = s.update({ effects: [setHolder.of("map"), setAnchors.of({ kind: "nodes", at: [2], anchor: 2 })] }).state;
  assert.deepEqual(highlightRanges(root), [{ from: 0, to: 16 }]); // 根は子孫まで
  const card = make("# r\n\n## a\n\n```\nx\n```\n")
    .update({ effects: [setHolder.of("map"), setAnchors.of({ kind: "card", at: 11 })] }).state;
  assert.deepEqual(card.field(choice), { kind: "card", id: 4 });
