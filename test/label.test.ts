...
const box: core.Box = { node, parent: null, buried: 0, x: 10, y: 20, w: 100, h: 30, cards: [] };
const row = core.metrics.row.normal;

test("倍率 1 — 箱の左上から枠のぶん外へ。字は箱の pad から始まる", () => {
  ...
  assert.equal(p.height, row.h + LABEL_BORDER * 2);
  assert.equal(p.fontSize, row.px);
  assert.equal(p.padding, row.pad);
});
...
test("倍率 2 — 位置と字と余白は倍になり、枠は倍にならない", () => {
  ...
  assert.equal(p.fontSize, row.px * 2);
  assert.equal(p.padding, row.pad * 2);
});

test("字が箱より長ければ右へ伸びる。倍率が小さくても余白は最低ぶん残す", () => {
  const p = labelPlacement(box, { k: 1, tx: 0, ty: 0 }, 200);
  assert.equal(p.width, 200 + row.pad * 2 + LABEL_BORDER * 2);
