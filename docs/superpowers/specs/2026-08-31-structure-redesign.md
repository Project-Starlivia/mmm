...
flat な Branch（計画が前提とする形）から**役割型 + Node enum**へ転換することを決定した。
...
pub struct Wing {          // 翼 = 場所。占有者を問わず側を持つ（id は持たない）
  side : Side
  branch : Branch
}
...
pub enum Sign { Heading; Item }   // Implicit は入れない — setSign の引数型なので
...
- doc の id は番兵 1（親指定用）。Wing は id を持たない（翼は占有者の id で指す）
- 命名の裁定: `Implicit / Explicit`（暗黙に/明示に表現されている — 「飛びが綴り」の教義に
  一番忠実。Present/Absent が次点、Written/Unwritten は教義と半衝突で却下）、
  `frontmatter`（head は見出しと衝突）、`Node`（略さない）、`sign` は据え置き

### 型で死んだもの（旧 check の住人）

doc の汚れ / 深いノードの side / 側つきで綴り無し / implicit×label / implicit×body /
implicit×folded / implicit×Item / setSign(Implicit) / **sides と children の整合**
（Wing が side と占有者を同じ要素で持つのでスプライスが原子的 — 並行配列問題は型で消滅）。
...
```
Path = Array[Int]     // [] = doc、[i] = root、[i, j] = 翼、それ以深 = children
Sub  = Whole(Root) | Part(Branch)   // 運搬の通貨。op.mbt の外に出ない

resolve(doc, id) -> Path?        // 腕なし
pluck(doc, path) -> Sub          // 3 腕（roots / wings / children から抜く）
graft(doc, parent, at, sub, side) -> Unit
                                 // 3 腕 + 変換の唯一の住所:
                                 //   doc へ: Part → Root 化（children を Wing(Right) で包む）
                                 //   root へ: Whole → 解体（sides は深さの物理で消滅）/ Part → Wing(side) で包む
                                 //   branch へ: Whole → 解体 / Part → そのまま
amend(doc, path, f : Node -> Node)   // fold / setSign 用（rename は読みの道なので通らない）
```

- **殺す条件の観測点**: 道具 4 つの腕が 3 で止まらなくなったら負け
- Whole は root 位置間では無変換（root の並べ替えで sides が無傷で旅する）
...
- 全 API 掃引済み: move（9 組合せ）/ delete / flipSide（root=鏡像・翼・深部 reject）/
  add / fold / setSign / serialize / parse / sig / project / id 写し — 全部この道具の合成で通る
...
type MapNode  = { id: number; label: string; implied: boolean;
                  folded: boolean; sign: "heading" | "item";
                  cards: Card[]; buried: number };
```

- project は mbt（法則 3）。バケツ分けは wings の filter — 側をまたぐ読み順はここで
  意図的に落ちる（絵に出ない情報）
...
| Wing をノード型にした連鎖 | children の型が段ごとに違い trait で書けない・move が変換表 |
| SidedWing / SidedImplied | 当初却下 → **トグル帰属の裁定反転で正当化**（側は隙間に付く） |
| Sign.Implied | 当初却下 → **Node enum で解決**（Sign は 2 値のまま、状態は Node が持つ） |

- **判定のリトマス**: 型分割を思いついたら move のシグネチャを書く。move が汚れる分割は
  操作の腹を横切っている。今回の採用形が通ったのは、Wing が「ノードの型」でなく
  「包み」で、深さ 3 以降が一様な Branch だから（変換は root 化/降格の 1 ペアだけ）
...
5. **sig（指紋）の形式再設計** — Doc/Root/Wing/Branch の走査に合わせる
6. **UI 翻訳層に 1 件追加** — バケツの index（左列の 2 番目）→ wings の index の写像
7. 保留事項なし — 「Root の複数列挙」は roots 配列 + Whole のままの splice で解決済み
