...
積んだ枠は捨てられていた。残るのは深さの stack（**高さが深さ**で、中身は「項目か」を
見るだけ）と裁定の控え。`fill` は `keep(… Card(…))` に潰れ、**裁定を下す所は全部 `keep`
1 つで見える**ようになった。

そこまで来ると `Build` は `build` と何の関係も無くなるので、`build.mbt` 914 行を仕事ごとに
割った。**綴りの行は 1 行も増えも減りもしていない。**

```
deem.mbt      md を読んで裁定を下す（`Build` → `Deem`、`lay` → `deem`）
assemble.mbt  ひと切れの列から木を組む（`Frame` / `Lay` / `to_node`）
measure.mbt   記法がどこまでかを原文から測る（見出しのラベル・項目のマーカーと詰め）
build.mbt     2 つを繋ぐ入口と、木の住所（`Spot` / `number` / `doc_id`）
```

### 真似るのは任意。**正規形が正解**（2026-09-11 に格下げ）

段 4 の決めの 2 つ目（**ぶれる 7 通りは、同じ文書の中の同じ関係を真似る**）を、決めから
任意に落とす。**基本は正規形の答えが正解**で、同じ関係が同じ文書に在るときだけ揃えてよい。

いまの実装はもう正解の側に居る。差す piece の隙間は正規形の答え（空行 1）で、項目の
兄弟だけ `loosened` が文書の流儀を写す（その doc が理由を書いている — 「正規形は流儀を
知らない」）。残る 7 通りへ広げるかは**任意の改善**で、やるなら出る字が変わるので
`loosened` と同じやり方で測ってから入れる。

```
書かれた         # r / 空行 / ## a / ### x     （## a → ### x は空行 0。書き手の流儀）
## b に子を差す   … / ## b / 空行 / ### y       （いまは空行 1。同じ関係が 2 通りの見た目になる）
```

**揃わないことは破れではない。** 正規形は mmm の答えとして正しく、揃えるのは
読みやすさの上乗せ。
