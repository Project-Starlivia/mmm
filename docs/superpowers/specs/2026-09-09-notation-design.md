...
### 残り — piece の抜き差しは 5a と一緒に

「piece を抜く」「差し替える」「差す」に訳す層は**まだ書かない。** 隙間の答えが
階層から出た今、その層が新しく持つのは「ぶれる 7 通りで同じ文書の同じ関係を真似る」
だけで、`plan` の 400 行を書き写して 5c で片方を消すことになる。piece を書き道に
するのは 5a（安全網と検証の入力を付け替える）と同じ瞬間なので、そこで一緒にやる。

網 — 見本 166 通り・fixtures 7 本・総当たり 1,700 通り・`merge_law_wbtest` が今のまま
通り、正規形 146 / 冪等 166 / 往復 166 / check 空 164 も動かない。
...
段 1 は足すだけ。段 2 と段 3 と段 4 は既存の試験が網（段 4 は物差しを試験の側に残した）。
...
段 4 は裏で走らせる必要が無かった。隙間は階層が言えるので、**古い道の答えと同じ字が
そのまま出る** — 突き合わせるのは字ではなく隙間の答えで、それは `gap_wbtest` が
正規形の物差しと総当たりで比べる（木 306 / 要素 960 / 違い 0）。

piece の抜き差し（5a）は字が変わり得るので、そこは裏で走らせて突き合わせる。
