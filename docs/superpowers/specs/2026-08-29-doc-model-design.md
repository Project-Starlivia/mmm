...
- **side を持てるのは骨格行を持つノードだけ**（implied は常に右）。
  スロットが implied のとき flipSide が来たら**昇格させてから反転**する
  — implied の綴りは level 飛びであって、飛びには側を書く場所が無い
  （「綴りは行き先に従う」の一般化。これを許すと `# r` + `#### b` の側が
  往復せず法則 1 が破れる）
...
| frontmatter | head 逐語。parse の前段で封筒として切り出す。
  文書頭の `---` は**閉じ delimiter と中身の形で frontmatter と裁定**し、
  該当しなければ先頭トグル（左開始）と読む — 判定規則の確定値は実装時に方言表へ |
