...
- **undo は CM history 1 本**。操作の editSets も打鍵も同じバッファに
  ...
  モデル用の履歴は持たない
- **初回スナップの告知**: 事前確認なし。整形後にトースト「全文を整形しました
  （undo で戻せます）」を出すだけ
- 全再パース性能は実測済みで安い（`docs/audit/PERF.md`: 5,000 ノード 191KB が 19.1ms）

## 8. 未決の旗

1. **list_from の細部** — 検出規則の基本線は「文書中で最も浅いリスト項目の深さ（下限 2）、
   リスト無しなら 7」。混在時の揃え方・エッジケースを詰め中

（スナップ告知・undo 統一は §7 で確定。reject と ghost の裁定は §5 で確定済み）
