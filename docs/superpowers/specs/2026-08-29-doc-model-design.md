...
- **Implicit が置ける位置** — 綴りとして読み戻せる位置だけ。見出しは後続の飛びを
  ...
  **Implicit の翼は、先頭であるか、直前の翼と側が違うこと**（側が違えばトグルが
  書かれ、それが境界になる）。深さ 3 以降の Implicit は先頭の子であること
  （そこにはトグルの綴りが無いため）。項目の後ろは吸収されないので常に置ける
...
- **flipSide は 3 通り**:
  ...
  **深いノード = Implicit の翼へ引き出す** — 自分を今の親から抜き、
