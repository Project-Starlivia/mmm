...
削除して G1 の `make_doc` / `make_branch` を呼ぶ形に、`write_tree` / `write_head` / `write_item` /
`write_gap` を `make_*` の合成に書き替えた。**組み上がる木も、書き出される md も、テスト本数 21 も
1 バイト・1 本も動かない**（`write_of` は `make_doc` と、`write_slot` は `make_branch` と定義が同一）。
...
3. **G4 が serialize へ足そうとしていた 3 本は、どれも要らない**（契約 §19 の G4 の表で削除済み）。
   ...
   - `spellable(kin, at)` — **不要**。`write_node` は兄弟の列も自分の index も受け取らないので
...
///|
/// 見出しのノード。畳まず・中身なし。
fn write_head(id : Int, label : String, children : Array[Node]) -> Node {
  make_node(id, make_head(label), children)
}

///|
/// 項目のノード。畳まず・中身なし。
fn write_item(id : Int, label : String, children : Array[Node]) -> Node {
  make_node(id, make_item(label), children)
}

///|
/// 飛び。骨格行を持たないので label も中身も型ごと無い。
fn write_gap(id : Int, children : Array[Node]) -> Node {
  make_node(id, Implicit, children)
}
...
test "C3: 見出しの兄弟は空行 1 本で継ぐ（右どうしなので区切りは 0 本）" {
  let doc = make_doc([
    write_tree(2, "r", [
      make_branch(Right, write_head(3, "a", [])),
      make_branch(Right, write_head(4, "c", [])),
    ]),
  ])
...
test "C1: 項目どうしの継ぎ目は詰める。空ラベルでも印と空白は書く" {
  ...
  let doc = make_doc([write_tree(2, "r", [make_branch(Right, a)])])
  assert_eq(serialize(doc), "# r\n\n## a\n\n- b\n- c\n- \n")
}
...
test "level は無制限。7 個以上の見出しも書く" {
  ...
  let doc = make_doc([write_tree(2, "r", [make_branch(Right, mid)])])
  assert_eq(
    serialize(doc),
    "# r\n\n## y\n\n### a\n\n#### b\n\n##### c\n\n###### d\n\n####### z\n",
  )
...
///|
fn write_node(pen : Pen, node : Node, depth : Int, steps : Int) -> Unit {
  write_skeleton(pen, node.skeleton, depth, steps)
  let inner = inner_steps(node.skeleton, steps)
  for child in node.children {
    write_node(pen, child, depth + 1, inner)
  }
}
...
- Consumes: `Block`（`Content` / `Rule` / `Opaque`）/ `Content`（`Image` / `Link` / `Code` / `Svg`）（G1）、
  `put` / `inner_steps` / `repeat`（Task 41）、`make_node` / `make_doc` / `make_branch`（G1）
- Produces:
...
test "C5: 中身は骨格行の下へ。飾りの水平線は *** で書く" {
  ...
  let doc = make_doc([write_tree(2, "r", [make_branch(Right, head2)])])
  assert_eq(
    serialize(doc),
    "# r\n\n## head2\n\n### head\n\ncontent01\n\n***\n\ncontent02\n",
  )
...
test "囲いは中身の最長のバッククォート連なりより 1 本長い" {
  ...
  let doc = make_doc([write_tree(2, "r", [make_branch(Right, n)])])
  assert_eq(serialize(doc), "# r\n\n## a\n\n````js\n```\nx\n````\n")
}
...
fn write_node(pen : Pen, node : Node, depth : Int, steps : Int) -> Unit {
  write_skeleton(pen, node.skeleton, depth, steps)
  let inner = inner_steps(node.skeleton, steps)
  write_body(pen, node.skeleton, inner)
  for child in node.children {
    write_node(pen, child, depth + 1, inner)
  }
}
...
- Produces:
  - `fn write_branches(pen : Pen, branches : Array[Branch], steps : Int) -> Unit`
...
test "C4: 先頭が左ならスロットの前に 1 本、変わり目にも 1 本" {
  let doc = make_doc([
    write_tree(2, "r", [
      make_branch(Left, write_head(3, "a", [])),
      make_branch(Right, write_head(4, "b", [])),
    ]),
  ])
  assert_eq(serialize(doc), "# r\n\n---\n\n## a\n\n---\n\n## b\n")
}
...
test "C16: 占有者が Implicit でも隙間にトグルは書ける" {
  ...
  let doc = make_doc([write_tree(2, "r", [make_branch(Left, deep)])])
  assert_eq(serialize(doc), "# r\n\n---\n\n#### b\n")
}
...
fn write_branches(pen : Pen, branches : Array[Branch], steps : Int) -> Unit {
  ...
  for branch in branches {
    if turned(prev, branch.side) {
      put(pen, Loud, steps, spell.toggle)
    }
    write_node(pen, branch.node, 2, steps)
    prev = Some(branch.side)
  }
}
...
test "項目の畳みも中身も、その項目の中身の列に入る" {
  ...
  let doc = make_doc([write_tree(2, "r", [make_branch(Right, x)])])
  assert_eq(
    serialize(doc),
    "# r\n\n- x\n\n  <details>\n\n  <summary>x</summary>\n\n  text\n\n  ```\n  1\n  ```\n  - y\n\n  </details>\n",
  )
...
fn write_node(pen : Pen, node : Node, depth : Int, steps : Int) -> Unit {
  write_skeleton(pen, node.skeleton, depth, steps)
  let inner = inner_steps(node.skeleton, steps)
  write_fold_open(pen, node.skeleton, inner)
  write_body(pen, node.skeleton, inner)
  for child in node.children {
    write_node(pen, child, depth + 1, inner)
  }
  write_fold_close(pen, node.skeleton, inner)
}
```
