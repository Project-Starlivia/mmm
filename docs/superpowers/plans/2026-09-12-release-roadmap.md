...
| #318 | 書く前に `lastModified` を見て、開いたときと違えば書かずに知らせる（「外部変更は追わない」は保ったまま、上書きだけ止める） | M |
| #300 | Recent の read-modify-write を 1 つの取引に。`find` の await も取引の中へ | M |
| #306 | **spec を先に書く**。控えは IndexedDB（Recent と同じ DB）、鍵は開いている文書 1 つ、立ち上がりの真ん中の一覧に並べて**勝手には開かない**。トグル `mmm.autosave` は捨て、名前のある文書はディスクへも常時書く | L |
| #310 | 狭い画面でも未保存の印は残す（名前だけ落とす）。置き場はロゴの隣 | S |
| #309 | Recent の行に `seen` の時刻と画像フォルダの名前を添える。同名は束ねない | S |
...
| ③#306 控えの置き場と復元 | IndexedDB・開いている 1 つ・真ん中の一覧に並べる・トグルは捨てる | 復元は勝手に開く／トグルは残す |
