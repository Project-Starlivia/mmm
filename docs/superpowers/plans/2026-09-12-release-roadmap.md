...
**進み（2026-09-13 時点）: 45 件 → 31 件**（#347 を新しく起票）。段 0 と段 A は済み、段 C から 6 件済み。
済んだものは行の頭に ✅ を付けてある。
...
| # | 何をする | 大きさ |
|---|---|---|
| ✅ #312 | 境界の定義から作り直した（[2026-09-15-border-design.md](../specs/2026-09-15-border-design.md)）。書き戻しで字が落ちる形は総当たり 69,904 通で 0。残る崩れは #377 / #378 | L |
| ✅ #315 | 行の無い箱が中身を抱える形は、読みから生まれなくなった（69,904 通で 0） | S |
| ✅ #314 | 法則の見本に「頭が水平線」6 本を足し、合流の破れ（項目の中の見出しの深さ・マーカーの行に乗る子・文書の頭の空行）を直した | M |
| ✅ #311 | 分けずに閉じた。parse の出力の破れは総当たり 69,904 通・無作為 20,000 通・境界の型 60,000 通で 0、反対向きの約束も法則（`check_law_wbtest`）にした | M |
| #316 | setext の下線の長さを綴りとして保つ（`marker` / `offset` / `loose` と同じ側）。改名の編集列を名前の行だけに | M |
| ✅ #319 | 項目の根も左右を持つ（#312 の作り直しで `ListSide` ごと消えた） | S |
| ✅ #381 | 列 0 の見出しが項目を閉じた後の行は、水平線と別の木（#387）。Implicit が根を埋める新しい木も側を持ち越さない | S |
| ✅ #382 | check が「書ける」と言う木は書ける（#389）。法則「check が通る木は書いて読み直すと同じ形」を無作為な木 20,000 本で。Flip side で戻すと右の最後の枝の尻（元の位置へは D&D か undo） | M |
| ✅ #383 | 貼る断片の側は書く側と同じ表で線に戻す（#391）。先頭の左を運び、畳みを壊さない | S |
| ✅ #384 | 境界をまたぐ持ち主の塗りと写し（#392）。境界の頭を地番に控える（`Spot.border`） | M |
| ✅ #385 | 境界の手前で足す・消す・名付けても合流は触った所だけ（#393） | M |
| ✅ 掃除 | 境界の作り直しの名残（#394）、合流の根の引数（#395）、書き戻しと隙間の計算を 1 本の歩きに（#396） | M |
| #377 | 文書の段の水平線で割れた項目の列が、書き戻しで 1 本に繋がり詰めが変わる（総当たり 69,904 通で 360） | S |
| #378 | 文書の頭に書き戻した水平線と後ろの setext の下線が frontmatter に読まれる（69,904 通で 4） | S |
| #386 | 書き戻しで記法が化ける小さな形（遅延継続行・名前の無い項目・字下げコード・文末の空白行・字下げした `<details>`） | M |
...
| #132 | 濃さは #346 で 3:1 に届いた。残りは `:focus-within` で出っぱなしにするかと、「輪」の語を焦点だけにするか。#347 と一緒に決める | S |
| #347 | **色の微調整をまとめて詰める**（作者の決め、2026-09-13）。#346 は基準に届かせるところまで。焦点の輪の重さ・箱の枠と根の枠・選んだアクセントでの強調の線・コードの配色の釣り合い・色の比を試験で見張るか | M |
