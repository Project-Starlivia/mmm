...
- **`write_of` と `write_slot` は削除**（`make_doc` / `make_branch` と 1 バイトも違わない）
- 残す 4 本は `make_*` の合成として書く（生の struct リテラルを書かない）。例:
  `fn write_head(id : Int, label : String, children : Array[Node]) -> Node { make_node(id, make_head(label), children) }`
...
| `find_in(node, id) -> Array[Int]?` | 0 | `resolve` の下請け |
| `node_at(doc, path) -> Node` | 1 | 深さ 2 以降のノードそのもの |
...
- `Sub` を変換する場所: **1 か所**
- 2 段包み（Branch と Node を両方作り直す枝）: **1 本**（`amend` の `[i, j]`）
- **判定: 合格**
...
- **警告**（設計は生きているが、次の一手を打つ）
  - どれかの道具が **4 腕**になった → `Branch` に `mut node` を 1 つ足して 3 へ戻せるか検討する
...
```moonbit
///|
/// 骨格を書き替える。容器 3 腕 — `[i, j]` だけは Branch と Node の 2 段を包み直す。
/// （`Branch` に `mut node` を 1 つ足せば 2 段包みが消える。腕が 4 本目になったら検討する）
```
...
fn resolve(doc : Doc, id : Int) -> Array[Int]? {
  for i, r in doc.roots {
    ...
    for j, b in r.branches {
      if find_in(b.node, id) is Some(tail) {
        let path = [i, j]
        path.append(tail)
        return Some(path)
      }
    ...
    Limb(n) =>
      {
        id: n.id,
        skeleton: n.skeleton,
        branches: n.children.map(fn(c) { { side: Right, node: c } }),
      }
  }
}
    ...
    Tree(r) =>
      {
        id: r.id,
        skeleton: r.skeleton,
        children: r.branches.map(fn(b) { b.node }),
      }
    Limb(n) => n
  }
}
...
interface MapBranch { node: MapNode; children: MapBranch[] }
interface MapTree   { node: MapNode; right: MapBranch[]; left: MapBranch[] }
...
- **バケツ分けは `branches` の filter。** `right` と `left` それぞれの中では branches の順序を保つ。
