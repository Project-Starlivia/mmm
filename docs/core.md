...
2. dirty なノードは、**span の範囲**をそのノードの正規形で置き換える
...
全文正規形へ落とす**から — 安全網の目が粗すぎる。比較は安い: id は読み時に
文書順で振られ、次の読みまで振り直さないので、前後の木は id で O(n) に
引き当たる。枝の移動は元の親と行き先の親の 2 箇所が dirty になり、枝自身は
両方にいるので「消えた・増えた」とは見ない(見るのは親の**子 id 列**)。

**全文の正規形と現在の md をテキストで diff する案は採らない。**
serialize(parse(md)) は恒等ではない(`*` と `-`、見出しの書き方、空行の数)ので、
触っていない箇所の「ずれ」まで差分に出て、操作のたびに文書全体が正規化される。
span で dirty の範囲だけ置き換えるのは、まさにそれを避けるため。
...
- **項目ノードを畳むとき `<details>` を何桁字下げして書くか。** 読みは決まった —
  項目の中に字下げして書けば畳みになり、容器の字下げは切り出しで剥がれる
  (字下げしなければ md 自身の規則で項目の外へ出る)。書く側の桁だけ未定
