...
- **apply は check を通る木しか返さない。** md に書けない並び（上の一覧）は、
  ...
  深ければ深さ 2 へ引き出して反対側）。形は読み直しが決め、そのとき focus は無い
  （`edit`）。合流も、読み直した形が「木を正規形に書いて読み直した形」と一致すれば
  全文に落とさない（`settled`）。貼り付けの `---` も、md ペインで末尾に書いた `---` の
  後ろに足す操作も、この 1 つの解釈に落ちる
