...
1. 操作の**前後の木を id で突き合わせて**、変わったノードを出す。
   操作は申告しない — label / body / fold / 子の id 列 のどれかが違えば dirty。
   **id は等しいかしか見ない**(reconcile が入ると id は文書順ではない)
...
5. 出来た文章を**読み直して、id を無視して形が一致するか検証**。違えば全文正規形へ落ちる。
   id を見ないのは、読みは振り直し、操作は新しい id を鋳るので、id で比べると必ず落ちるから
6. 編集リストを 1 トランザクションで渡す。カーソルと undo は CodeMirror の仕事

正しさは 5 の検証が持つ。だから 1〜4 がどれだけ雑でも壊れない。

1 が申告でなく比較なのは、**申告は操作ごとに手で書くもので、漏れると 5 が
全文正規形へ落とす**から — 安全網の目が粗すぎる。比較は安い: 前後の木は id で
O(n) に引き当たる。枝の移動は元の親と行き先の親の 2 箇所が dirty になり、
