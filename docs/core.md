...
md に書けない並びは 6 つ。
- 行の無い根（Implicit）の最初の子が左（`Ownerless`）— 手前の `---` は文書の段に書かれ、
  持ち主が文書なので境界にならない（spec.md「水平線」の `--- / ## a`）。根を綴れば書ける
- ノードの中身の尻の水平線に、同じ木の骨格の行が続く（`BorderBreak`）— 境界（側の
  変わり目）に読まれる。境界になるのはノードの body に居る水平線だけなので、文書の
  body の水平線と、直後が新しい木（深さ 1）の水平線は咎めない（spec.md「水平線」）。
  子が項目だけなら、尻に続く水平線は子の後ろに書く（項目の列を閉じた塊は持ち主に戻る）ので、
  続くのは部分木の次の骨格の行。項目の子の列の尻の線に見出しの子が続くなら、中身を間に挟む
  （`middle` — 書く側の `strokes` も同じ答えを引く）。
  境界は根より深い行を閉じて直後の行を反対側へ引き出すので、根の子より深い列でも読みは変わる。
  **これだけは apply が通す** — 書けば md が境界と読むので、形は読み直しが決める
  （「操作」）。**側が変わる所は破れではない** — 境界の `---` がもう 1 本書かれるので、
  読みは 1 本目を中身に落として 2 本目を境界にする
- 見出し（綴られた・綴られない）に続く項目、境界なしで続く Implicit（`Swallowed`）—
  項目はその見出しの子に読まれ、Implicit の飛びはその見出しの深さから測られる。
  **境界で逃げられるのは Implicit だけ** — 項目の深さは書かれた入れ物から測るので、
  `## a / --- / - b` の b は a の兄弟にならず、境界の後ろの Implicit の下に読まれる。
  **畳まれた見出しも飲み込まない** — `</details>` がその部分木の終わりを言う
- Implicit の**先頭の子**が項目（`GappedItem`）— 項目の深さは書かれた入れ物から読まれる。
  Implicit には行が無いので、先頭の項目は 1 つ浅く読まれ、Implicit が消える。
  2 つ目以降は直前の兄弟が深さを留める（見出しの直後なら `Swallowed` が裁く）。
  **境界の直後に立つ Implicit は（`## head / --- / - head` の `() → [head]`）
  この限りではない** — 境界より前の行の節か入れ物の中に `---` と項目を書けば、項目の深さは
  そこから測られて保たれる（spec.md「水平線」。どこに書けるかは書きの段で決める）
...
**畳みの閉じは部分木の終わり。** 領域は部分木を過不足なく覆うので、`</details>` の後ろは
定義上その部分木の外（行も中身も。`---` はここが違い、中身の持ち主を閉じない）。`sift` が閉じの位置に印（`Closed`）を残し、積む側（`shut`）が
...
md を全部集めた見本（188 通り）を、正規形・冪等・往復・破れの 4 つで押さえる。木の形
...
範囲は**骨格の行の頭から、自分と子孫の中身の一番後ろの尻まで**。尻は最後の子とは
限らない — 境界は中身の持ち主を変えないので、子の中身が境界で置き直された後ろの行より
下に書かれていることがある（`## a / --- / - b / text` の text は a のもの）。
次の骨格の頭までは伸ばさない — 伸ばすと隣の空行や側の `---` を抱え込み、
書き直したときに食ってしまう。

**範囲は外の物を跨ぎうる。** 置き直された外の行や、外の持ち主の中身が中に書かれていれば、
塗りと写し（`read` の `Survey::cover`）はそこを抜いて割る。

- **境界の頭（`border`）** — 境界の直後に座る根の子だけが持つ、手前の水平線の行の位置。
  合流が境界の行だけを消すのと、写しが置き直された行を手前の線ごと抜くのに使う
  （原文を読み直して水平線を探さない）
...
空行、行頭の飾りの空白、子より後ろに書かれた中身の並び）ので、触っていない箇所の
「ずれ」まで差分に出て、操作のたびに文書全体が正規化される。
...
- **境界の `---` は行 1 本ぶんの要素。** 側は根のもので、`---` は側が変わる所にだけ書かれる。
  側が変われば、残る子で要らなくなった `---` の行だけを消し、要る所にだけ足す（根ごと書き直さない）。
  **線は直後の子の隙間に入る** — 子を消せば線ごと消え、子の手前に差す物は線より前に入り、
  手前の子を消しても線は残る
- **名前の無い行に名前が付いただけなら行を 1 本差す。** Implicit を綴った見出しは、子の並びが
  同じなら部分木ごと書かず、行を差して子へ降りる
...
境界の直後の根の子は、手前の境界の行とその前の空行までが隙間（`gaps` / `lead` — 地番の `border`）。
...
- **差すもの・運ぶものは形だけを持ち込み、綴りは席が決める（`sign_at`）。** 新しい
  ...
  （`spell_at`。番号付きの隣なら 1 つ進める）。**種類も綴りも同じ相手から借りる** —
  隣が居なければ親（`follows`。前は種類だけ親を継いで綴りが既定に落ち、`* a` の下に
  `- n` が書かれて同じ列にマーカーが混ざった。#243）。読んだ列の綴りは保つ。モードは Reform と
...
- **flipSide は 3 段。** 根は `sides` を一括反転(鏡像)、根の子はその 1 つを反転、
  深いノードは反対側の末尾へ引き出す(深さは保つ)。見出しも項目も、見出しの根も
  項目の根も同じ — 読みの境界と同じ形。**動くのは選んだノードだけ** — 行き先の側の
  最後の子の後ろに `---` が書かれるなら間を Implicit が埋め、書かれない(同じ側)なら
  Implicit は立てず、深さのままその子の部分木の尻へ入る(md の読みどおりの親)。
  元の側に残った Implicit は子が尽きれば `mend` が消す
...
- **apply は check を通る木しか返さない。** md に書けない並び（上の一覧）は、
  ...
  足す操作も、この 1 つの解釈に落ちる。**貼る断片の側**は、書く側と同じ境界の表で
  中身の水平線に戻して運ぶ（`@tree.bordered`）— 変わる子の行の直前に中身の尻が書かれる
  行の尻へ置き、置ける行が無い変わり目（畳みの中しか無い・子が項目だけの根の最初の子）は落とす
