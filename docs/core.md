...
変わったのが自分だけ（子の並びは同じ）なら、書くのは**自分の行と中身の範囲**
（最初の子の行頭まで）だけで、子は子列の差分に任せる。ラベルを変えても子孫は動かない。
...
- **中身を消した focus は持ち主。** ノードを消した focus は無い（隣を選ぶのは UI
  の仕事）が、中身を消した focus は持ち主（消えていなければ）。反映は変わった
  ノードを持ち主ごと書き直すので、消える前の中身に置いた目印は生き残れない
