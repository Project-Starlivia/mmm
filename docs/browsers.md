...
mmm は「**Chromium 系でしか動かない**と言い切る」と決めている（`spec.md`）。この文書は、
...
**だからこの言い切りは、決めた当時より正しい。** 揃うのを待たない
...
**シェルもファイルの話をしない** — (b′) は #113 で入った。

- **帯は拡張子を付けない** — 持てないなら `xxx.md` ではなく文書の名前だけを出す。
  押せば理由を言う（`aria-disabled` と、Files の Rename と同じ title）
- **未保存の `●` は点かない** — 消える手段が無いものを点けない。**条件はこの
  能力ひとつで、幅は加わらない** — 狭い画面で退くのは名前だけ（タブの題は
  未保存を言わないので、印まで退けば言うものが無くなる）
- **閉じるときも止めない** — 解消する手段が無いのに毎回聞かない
- **`.md` のドロップは理由を言う** — 落ちたのに 1 つも札にならなければ、そう言う
  （フォルダを落としたときも札はフォルダのものなので、同じ道で言う）

**判断は 1 つに寄せてある** — `app/app.mbt` の `App::can_hold`（保存の入口が
あるか、または既にファイルを持っているか）。帯・印・警告が同じ問いを見る。

ここに挙げていた「ファイル名のクリックが無言の空振り」は、先に #53（`cfd2827`）で
直っていた — (b′) の枝ではなく、部品を並べる作業の道連れ。

## 線の外で何をするか — 3 つの段

| | 何ができる | 要るもの | リロードで |
|---|---|---|---|
| **(b′) 正直な機能オフ** | 書く・書き出す | 帯 / `●` / beforeunload / ドロップの無言を本当のことにする | 消える（と言ってある） |
...
それをやっていないのは、読めても書き戻せない文書ができるからで、これは「動く道は 1 本」の
...
1. **Chromium 専用は維持する。ランタイムのフォールバックは持たない。**
   Chromium 限定の決定は覆さない — 理由は当時より強くなった
2. **(b′) は済んだ（#113）。** Task 10 と同じ原則（無いものを黙って落とさない）を、
   言い忘れていた 4 か所に当てた。新機能ではなく、やりかけの完成
