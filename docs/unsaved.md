...
| 見せる | Files の `Open Unsaved ▸` に新しい順。**いま開いている文書自身の件は出さない**。開くと `●` を立てたまま戻し、**元の件を引き受ける**（同じ字の件を増やさない）。札付きの件は**ディスクの字と比べる**（比べるためだけに読む。同じなら件を捨てる） |
...
| 複数のタブ | **未対処。** 開き直した件は鍵を引き受けるので、同じ件を 2 つのタブで開くと後から書いたほうが勝ち、片方の保存や Don't Save でもう片方の件も消える。同じファイルを普通に 2 つのタブで開いたときは、保存時の上書きの確かめ（#318）で気づける |
