...
コードはこの決めのとおり（#373 の 4 段: 名前・中身・いつ書くか・書けなかったとき）。
今の動きは `spec.md`。
...
| 捨てる | **ディスクと同じ字になった**とき（保存・別名で保存・undo で戻る・空に戻す。書いている途中でも）／**Don't Save を選んで文書を入れ替えた**とき（入れ替わらなければ残す）／20 件を超えた古いもの／Discard all unsaved（並んでいるものだけ。いまの文書の件は残す） |
| 残す | **選ばずに消えた**とき（タブやブラウザが落ちた・閉じた・再読み込みした）。閉じるときのブラウザの「離れますか」は、mmm にはどちらを押したか分からないので残す |
| 見せる | Files の `Open Unsaved ▸` に新しい順。**いま開いている文書自身の件は出さない**。開くと `●` を立てたまま戻し、**元の件を引き受ける**（同じ字の件を増やさない）。札付きの件は**ディスクの字と比べる**（比べるためだけに読む。同じなら件を捨てる）。**上書きの確かめの基準は、件を書いたときのディスクの時刻に戻す**（その後にほかで保存されていれば、保存するときに上書きするか聞く。VS Code #15749 と同じ形。基準を持たない件は確かめない） |
| 保持 | 件数で 20。巨大な文書 1 つが棚を食い尽くさないよう、合計 20 MB の柵は残す（柵より大きい 1 件は書かず、書けなかったと言う）。日数の期限は置かない |
...
そのぶん、たずねの補足（前の `Unsaved changes are autosaved.`）は消した。
**捨てるのは文書の入れ替えが決まってから** — たずねの時点で捨てると、開く相手に断られた
（ピッカーを閉じた・許可を断った）ときに、汚れた文書だけが画面に残る。
...
| ディスクの基準 | 件を書いたとき、ディスクを最後に読んだ・書いた時刻 | `stamp`（`Disk` の `stamp` と同じ語） | — |
| 最後に書いた字 | 書いた字（同じ字を二度書かないため） | `written`（前の `kept`） | — |
| 書く | 書く（いまの件を上書き） | `keep` | — |
| 引き受ける | 開き直した件を引き受ける | `adopt`（`Disk::adopt` と同じ語。`resume` は MoonBit の予約語） | — |
| 捨てる相手が居るか | いまの件が在るか、書いている途中か | `held` | — |
| いまの件のほか | — | `others` | `Open Unsaved ▸` に並ぶ行 |
| 開く | 未保存の文書から開く | `open_unsaved` | `Open Unsaved ▸` |
| 1 件を捨てる | 捨てる（鍵も振り直す。文書を入れ替えるときも、ディスクと同じ字になったときも） | `discard` | —（人が押す口は Don't Save） |
| 並んでいるものを捨てる | 並んでいるもの（ほかの件）を全部捨てる | `discard_others` | `Discard all unsaved` |
...
| 複数のタブ | **見送る（稀なので）。ロックはしない** — ほかのアプリでも同じファイルは触られるので、タブ同士だけを特別扱いする理由が薄い。開き直した件は鍵を引き受けるので、同じ件を 2 つのタブで開くと後から書いたほうが勝ち、片方の保存や Don't Save でもう片方の件も消える。同じファイルを普通に 2 つのタブで開いたときは、保存時の上書きの確かめ（#318）で気づける |
...
- 1 つの鍵を後から書いたほうが勝つ作りは、他のタブの字を消す（Excalidraw #10770、未解決）。
  mmm は開くたびに鍵を振るが、**開き直した件は鍵を引き受ける**ので、同じ件を 2 つのタブで
  開けばこの形が起きる（未対処）
